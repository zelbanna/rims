...
echo "isc-dhcp-server: Connect to host"
echo ""
echo " - press enter to continue - "
read
docker create --name=mariadb -e MYSQL_ROOT_PASSWORD=$mariadbpassword -e TZ=Europe/Stockholm -v mariadb:/config --net services --ip 172.18.0.2 --restart unless-stopped linuxserver/mariadb
docker start mariadb
docker create --name=influxdb2 -p 8086:8086/tcp -v influxdb2:/var/lib/influxdb2 -v influxdb2.conf.d:/etc/influxdb2 --net services --ip 172.18.0.3 --restart unless-stopped influxdb
docker create --name=pdns-server -v pdns-server:/etc/powerdns  -v /var/sockets/pdns-server:/var/run/pdns --net services --ip 172.18.0.5 --restart unless-stopped xddxdd/powerdns
...
docker create --name=telegraf -p 8092:8092/udp -p 8125:8125/udp -p 8094:8094/tcp -v telegraf:/etc/telegraf -v /var/run/docker.sock:/var/run/docker.sock -v /var/sockets:/var/sockets --net services --ip 172.18.0.7 --restart unless-stopped telegraf
docker create --name=dhcpd --net host -v dhcpd:/data --restart unless-stopped networkboot/dhcpd eth0
