...
def list(aWeb):
 ...
 for row in rows:
  ...
  if int(cookie['id']) == row['user_id'] or row['valid'] == 0:
   ...
   print aWeb.button('delete', DIV='div_content', TITLE='Remove reservation', URL='zdcp.cgi?reservations_list&op=drop&device_id=%i&user_id=%i'%(row['device_id'],row['user_id']))
  print "&nbsp;</DIV></DIV>"
