...
def domain_info(aWeb):
 ...
 if not data['id'] == 'new':
  aWeb.wr("<label for='foreign_id'>Foreign ID:</label><span id='foreign_id'>%s</span>"%(res['infra']['foreign_id']))
 aWeb.wr("</DIV>")
 aWeb.wr("</FORM>")
 ...
 if data['id'] == 'new':
  ...
  aWeb.wr(aWeb.button('reload',DIV='div_content_right',URL='dns_domain_info?id=%s'%(data['id'])))
  aWeb.wr(aWeb.button('trash',DIV='div_content_right',URL='dns_domain_delete?id=%s'%data['id']))
 aWeb.wr("</ARTICLE>")
