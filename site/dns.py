...
def domain_info(aWeb):
 if aWeb['op'] == 'update':
  ...
  res = aWeb.rest_generic(PC.dns['url'], "{}_domain_update".format(PC.dns['type']),data)
  data['id'] = res['id']
 else:
  res = aWeb.rest_generic(PC.dns['url'], "{}_domain_lookup".format(PC.dns['type']),{'id':aWeb['id']})
  data = res['data']
...
def domain_transfer(aWeb):
 domains = aWeb.rest_generic(PC.dns['url'], "{}_domains".format(PC.dns['type']),{"filter":"arpa"})
 print "<ARTICLE>"
...
#
def records(aWeb):
 dns = aWeb.rest_generic(PC.dns['url'], "{}_records".format(PC.dns['type']),{'domain_id':aWeb['id']})
...
def record_info(aWeb):
 if aWeb['op'] == 'update':
  data = aWeb.get_args2dict_except(['call','op'])
  res = aWeb.rest_generic(PC.dns['url'], "{}_record_update".format(PC.dns['type']),data)
  data['id'] = res['id']
 else:
  res = aWeb.rest_generic(PC.dns['url'], "{}_record_lookup".format(PC.dns['type']),{'id':aWeb['id'],'domain_id':aWeb['domain_id']})
  data = res['data']
 lock = "readonly" if not data['id'] == 'new' else ""
...
def record_delete(aWeb):
 res = aWeb.rest_generic(PC.dns['url'],"{}_record_delete".format(PC.dns['type']),{'id': aWeb['id']})
...
def top(aWeb):
 ...
 dnstop = aWeb.rest_generic(PC.dns['url'], "{}_top".format(PC.dns['type']), {'count':20})
...
#
def dedup(aWeb):
 dns = aWeb.rest_generic(PC.dns['url'], "{}_dedup".format(PC.dns['type']))
