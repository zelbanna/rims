...
def report(aWeb):
 ...
 for rep in resp['data']['Metadata']:
  ...
  if report['ProjectId'] == pid:
   ...
   print "<DIV CLASS=z-table2 style='width:99%; display:inline-block;'><DIV CLASS=tbody>"
   for ent in report['Data']:
    #print "<THEAD><TH>Field</TH><TH>Data</TH></THEAD>"
    print "<!-- Ent -->"
    for key,value in ent.iteritems():
     if isinstance(value,dict):
      print "<DIV CLASS=tr><DIV CLASS=td>{}</DIV><TD style='white-space:normal; overflow:auto;'><DIV CLASS=z-table2><DIV CLASS=tbody>".format(key)
