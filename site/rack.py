...
def main(aWeb):
 with DB() as db:
  ...
  racks = db.get_rows()
 print "<CENTER><H1>Rack Overview | <A CLASS=z-op DIV=div_main_cont URL=sdcp.cgi?call=device_main&target=vm&arg=1>Virtual Machines</A></H1><BR>"
 rackstr = "<A TITLE='{1}' CLASS=z-op DIV=div_main_cont URL=sdcp.cgi?call=device_main&target=rack_id&arg={0}><IMG ALT='{1} ({2})' SRC='images/{2}'></A>&nbsp;"
...
def info(aWeb):
 ...
 print "</DIV></DIV>"
 print "</FORM>"
 print "<A TITLE='Reload info' CLASS='z-btn z-op z-small-btn' DIV=div_content_right URL=sdcp.cgi?call=rack_info&id={0}><IMG SRC='images/btn-reboot.png'></A>".format(id)
