...
def clear(aWeb):
 ...
 print "<ARTICLE>"
 for host in hosts:
  res = aWeb.rest_call("http://%s/rest.cgi"%host,'sdcp.rest.examine_clear_logs',{ 'logs':[PC.generic['logformat'],PC.generic['netlogs']]})['data']
...
#
def logs(aWeb):
 res = aWeb.rest_call("http://{}/rest.cgi".format(aWeb['ip']),'sdcp.rest.examine_get_logs',{'count':18,'logs':[PC.generic['logformat'],PC.generic['netlogs']]})['data']
