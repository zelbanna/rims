...
def shutdownall(aWeb):
 ...
 if domain and srvlist:
  for host in srvlist:
   fqdn = host+"."+domain
   t = Thread(target = thread_shutdown_host, args=[fqdn])
   t.start()
 else:
...
def rack(aWeb):
 ...
 print "<A CLASS='z-btnop' OP=load DIV=div_navleft LNK='devices.cgi?domain={0}&view=devicelist&target=rack&arg={1}'>Devices</A>".format(domain,rack)
 if con:
  print "<A CLASS='z-btnop' OP=load DIV=div_navleft LNK='devices.cgi?domain={0}&view=consolelist&conlist={1}'>Console</A>".format(domain,con)
 if pdu:
  print "<A CLASS='z-btnop' OP=load DIV=div_navleft SPIN=true LNK='devices.cgi?domain={0}&view=pdulist&pdu={1}'>Power</A>".format(domain,pdu)
 ...
 print "<DIV CLASS=z-nav_left  ID=div_navleft></DIV>"
 print "<DIV CLASS=z-nav_right ID=div_navcont>"
 rack_info(aWeb)
 print "</DIV>"
...
def esxi(aWeb):
 ...
 print aWeb.get_listeners("div_nav_cont")
 print "<DIV CLASS='z-navcontent' ID=div_nav_cont>"
