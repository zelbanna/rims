...
def main(aWeb):
 ...
 print "<NAV><UL>"
 print "<LI><A CLASS=z-op DIV=div_content URL='zdcp.cgi?multimedia_list'>Media files</A></LI>"
 print "<LI CLASS='dropdown'><A>Services</A><DIV CLASS='dropdown-content'>"
 for svc in svcs.get('services',[]):
  print "<A CLASS=z-op DIV=div_content URL='zdcp.cgi?tools_services_info&node=%s&service=%s'>%s</A>"%(aWeb.id,svc['service'],svc['name'])
 print "</DIV></LI>"
 print "<LI><A CLASS='z-op reload' DIV=main URL='zdcp.cgi?multimedia_main'></A></LI>"
...
def list(aWeb):
 ...
 print aWeb.button('reload',DIV='div_content', URL='zdcp.cgi?multimedia_list')
 print aWeb.button('trash', DIV='div_content_right', URL='zdcp.cgi?multimedia_cleanup', MSG='Are you sure?')
 ...
 for row in data['files']:
  ...
  print aWeb.button('info',     DIV='div_content_right', TITLE='Title info', URL='zdcp.cgi?multimedia_title&path=%s&file=%s'%(row['path'],row['file']))
  print aWeb.button('search',   DIV='div_content_right', TITLE='Lookup info',URL='zdcp.cgi?multimedia_lookup&path=%s&file=%s'%(row['path'],row['file']))
  print aWeb.button('document', DIV='div_content_right', TITLE='Subtitles',  URL='zdcp.cgi?multimedia_subtitles&path=%s&file=%s'%(row['path'],row['file']))
  print aWeb.button('trash',   DIV='div_content_right', TITLE='Delete file',URL='zdcp.cgi?multimedia_delete&path=%s&file=%s'%(row['path'],row['file']))
  print "</DIV></DIV></DIV>"
...
def title(aWeb):
 ...
 print aWeb.button('start',DIV='div_content_right', SPIN='true', URL='zdcp.cgi?multimedia_process',  FRM='multimedia_info_form')
 print aWeb.button('sync', DIV='div_content_right', SPIN='true', URL='zdcp.cgi?multimedia_transfer', FRM='multimedia_info_form')
 print "</DIV></ARTICLE>"
...
def process(aWeb):
 ...
 data = aWeb.rest_call("multimedia_process",args, 360)['data']
 print "<ARTICLE CLASS=info><P>%s</P>"%aWeb['file']
 print "<DIV CLASS=table><DIV CLASS=tbody>"
