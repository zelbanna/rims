...
def list(aWeb):
 ...
 if not token:
  ...
  return
 data = aWeb.rest_call("openstack/call",{'token':token,'service':'contrail','call':"virtual-networks?fields=name,display_name,virtual_network_properties,network_ipam_refs"})['data']
...
def action(aWeb):
 ...
 if   op == 'info':
  ...
  vn = aWeb.rest_call("openstack/call",args)['data']['virtual-network']
  ...
  vn = aWeb.rest_call("openstack/contrail_interfaces",args)
 ...
 #
  ...
  args['virtual_network'] = id
  fips = aWeb.rest_call("openstack/contrail_floating_ips",args)
  ...
  args['uuid'] = aWeb['id']
  res = aWeb.rest_call("openstack/contrail_uuid",args)
  aWeb.wr("<PRE>{}</PRE>".format(dumps(res,indent=4)))

 elif op == 'remove':
  args['call'] = "virtual-network/{}".format(id)
  args['method'] = 'DELETE'
  res = aWeb.rest_call("openstack/call",args)['data']
  ...
  args['service'] = 'nova'
  args['call'] = "servers"
  vms = aWeb.rest_call("openstack/call",args)['data']['servers']
  ...
  args['vm'] = vm_id
  vmis = aWeb.rest_call("openstack/contrail_vm_interfaces",args)
