   ...
   if index == rackinfo['size'] and count == 1:
    ...
    if rowspan > 0:
     ...
     if rackunits.get(count*index,None):
      ...
      print "<TD CLASS='z-rack-data' rowspan={2} style='background-color:green'><CENTER><a class='z-op' title='Show device info for {0}' op='load' div='div_navcont' lnk='ajax.cgi?call=device_device_info&devices_id={1}'>{0}</a></CENTER></TD>".format(rackunits[count*index]['hostname'],rackunits[count*index]['id'],rowspan)
      rowspan = rowspan - 1
     else:
