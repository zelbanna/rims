...
def list(aWeb):
 ...
 print aWeb.button('reload', DIV='div_content', URL='sdcp.cgi?users_list')
 print aWeb.button('add',    DIV='div_content_right',URL='sdcp.cgi?users_info&id=new')
 ...
 for row in rows:
  print "<DIV CLASS=tr><DIV CLASS=td>{0}</DIV><DIV CLASS=td><A CLASS='z-op' DIV=div_content_right URL='sdcp.cgi?users_info&id={0}'>{1}</A></DIV><DIV CLASS=td>{2}</DIV><DIV CLASS=td>{3}</DIV></DIV>".format(row['id'],row['alias'],row['name'],row['email'])
...
def info(aWeb):
 ...
 print aWeb.button('save',DIV='div_content_right', URL='sdcp.cgi?users_info&op=update', FRM='user_info_form')
 if data['id'] != 'new' and ((cookie['id'] == str(data['id']) or cookie['id'] == "1")):
  print aWeb.button('trash',DIV='div_content_right',URL='sdcp.cgi?users_delete&id={0}'.format(data['id']), MSG='Really remove user?')
 print "</DIV>"
