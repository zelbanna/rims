...
def info(aWeb):
 ...
 aWeb.wr("<DIV>Right/Left slots:</DIV><DIV>%s</DIV>"%("True" if pdudata['slots'] == 2 else "False"))
 for slot in range(0,pdudata['slots']):
  aWeb.wr("<DIV>Slot %s Name:</DIV><DIV>%s</DIV>"%(slot,pdudata['%s_slot_name'%slot]))
  aWeb.wr("<DIV>Slot %s ID:</DIV><DIV>%s</DIV>"%(slot,pdudata['%s_slot_id'%slot]))
 aWeb.wr("</DIV>")
...
def unit_info(aWeb):
 ...
 aWeb.wr("<DIV CLASS='info col2'>")
 aWeb.wr("<DIV>Slot.Unit:</DIV><DIV>{0}.{1}</DIV>".format(aWeb['slotname'],aWeb['unit']))
 aWeb.wr("<DIV>Text:</DIV><DIV><INPUT NAME=text TYPE=TEXT PLACEHOLDER='{0}'></DIV>".format(aWeb['text']))
 aWeb.wr("</DIV>")
