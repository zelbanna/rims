...
def list(aWeb):
 ...
 print aWeb.button('reload', DIV='div_content', URL='sdcp.cgi?bookings_list')
 ...
 for row in rows:
  print "<DIV CLASS=tr><DIV CLASS=td><A CLASS='z-op' DIV=div_content_right URL='sdcp.cgi?users_info&id={3}'>{0}</A> ({3})</DIV><DIV CLASS=td><A CLASS='z-op' DIV=div_content_right URL='sdcp.cgi?device_info&id={4}'>{1}</A></DIV><DIV CLASS='td {5}'>{2}</DIV><DIV CLASS='td controls'>".format(row['alias'],row['hostname'],row['end'],row['user_id'],row['device_id'],'' if row['valid'] == 1 else "orange'")
  if int(cookie['id']) == row['user_id'] or row['valid'] == 0:
   print aWeb.button('add',    DIV='div_content', TITLE='Extend booking', URL='sdcp.cgi?bookings_list&op=extend&device_id=%i&user_id=%i'%(row['device_id'],row['user_id']))
   print aWeb.button('delete', DIV='div_content', TITLE='Remove booking', URL='sdcp.cgi?bookings_list&op=debook&device_id=%i&user_id=%i'%(row['device_id'],row['user_id']))
  print "&nbsp;</DIV></DIV>"
