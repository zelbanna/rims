...
def list(aWeb):
 ...
 for row in rows:
  ...
  if int(aWeb.cookie.get('sdcp_id')) == row['user_id'] or row['valid'] == 0:
   print "<A CLASS='z-btn z-small-btn z-op' DIV=div_content_left TITLE='Extend booking' URL='sdcp.cgi?call=bookings_list&op=extend&device_id={}&user_id={}'><IMG SRC='images/btn-add.png'></A>".format(row['device_id'],row['user_id'])
   print "<A CLASS='z-btn z-small-btn z-op' DIV=div_content_left TITLE='Remove booking' URL='sdcp.cgi?call=bookings_list&op=debook&device_id={}&user_id={}'><IMG SRC='images/btn-remove.png'></A>".format(row['device_id'],row['user_id'])
  print "&nbsp;</DIV></DIV>"
 print "</DIV></DIV></DIV>"
