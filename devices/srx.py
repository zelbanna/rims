...
class Device(Junos):
 ...
 #################################### Authentication ####################################
 #
 def cp_auth_table(self):
  try: res = self._router.rpc.get_authentication_table_cp({'format':'json'})['authentication-table-cp'][0]
  except Exception as e:
   return {'status':'NOT_OK','info':str(e)}
  else:
   table = [{'ip':auth['source-ip'][0]['data'],'alias':auth['user-name'][0]['data'],'roles':auth['ref-group-name-list'][0]['ref-group-name'][0]['data'].split(', ')} for auth in res['ad-auth-table'][0]['ad-authentication-info']] if res.get('ad-auth-table') else []
   return {'status':'OK','data':table}

 def userfw_auth_table(self):
 ...
 def userfw_auth_add(self, aAlias, aIP, aRoles):
 ...
 def userfw_auth_delete(self, aAlias, aIP):
