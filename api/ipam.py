...
def address_events(aCTX, aArgs):
 ...
 with aCTX.db as db:
  if aArgs.get('op') == 'clear':
   ...
   ret['count'] = db.query("SELECT {} FROM {} WHERE {} ORDER BY time DESC LIMIT {} OFFSET {}".format(", ".join(fields), " LEFT JOIN ".join(joins), filter, aArgs.get('limit','50'), aArgs.get('offset','0')))
