...
def status(aRT, aArgs):
 ...
 ret = {}
 try:
  command = aRT.config['services']['openvpn']['status']
...
def restart(aRT, aArgs):
 ...
 """
 ret = {}
 ret['stop'] = __op(aRT.config['services']['openvpn']['stop'])['status']
 sleep(2)
 ret.update(__op(aRT.config['services']['openvpn']['start']))
 return ret
...
def parameters(aRT, aArgs):
 ...
 Output:
  ...
  - parameters
 """
 settings = aRT.config['services'].get('openvpn',{})
...
def start(aRT, aArgs):
 ...
 Output:
  - status
 """
 return __op(aRT.config['services']['openvpn']['start'])
...
def stop(aRT, aArgs):
 ...
 Output:
  - status
 """
 return __op(aRT.config['services']['openvpn']['stop'])
