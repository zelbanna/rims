 ...
 with DB() as db:
  ...
  if ret['type'] == 'map':
   ...
   for lvl in range(0,args.get('diameter',2)):
    ...
    for from_id in level_ids:
     from_dev = devices[from_id]
     if not from_dev['processed']:
      ...
      for con in cons:
       ...
       #  -- in case already found as new, it means there is already such a connection, find that one and make interfaces 'multihomed' ('dynamic')
       ...
       # 2) For multipoint the other side will (through SQL) see a straight interface so also covered by processed
