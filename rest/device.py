...
def info(aCTX, aArgs = None):
 ...
 with aCTX.db as db:
  ...
  if op == 'lookup':
   db.do("SELECT INET_NTOA(ia.ip) AS ip FROM ipam_addresses AS ia LEFT JOIN device_interfaces AS di ON di.ipam_id = ia.id LEFT JOIN devices ON devices.management_id = di.interface_id WHERE devices.id = '%s'"%ret['id'])
   ...
   if aArgs.get('class') == 'vm':
    db.do("UPDATE device_interfaces SET class = 'virtual' WHERE device_id = %i"%ret['id'])
   ret['update'] = (db.update_dict('devices',aArgs,"id=%s"%ret['id']) == 1)

  ret['found'] = (db.do("SELECT * FROM devices WHERE id = %s"%ret['id']) == 1)
  if ret['found']:
   ret['data'] = db.get_row()
...
#
#
def interface_connect(aCTX, aArgs = None):
 ...
 with aCTX.db as db:
  ...
  if not aArgs.get('disconnect',False):
   ...
   id = db.get_last_id()
   ret['update'] = db.do("UPDATE device_interfaces SET connection_id = %s WHERE interface_id IN (%s,%s)"%(id,aArgs['a_id'],aArgs['b_id']))
 return ret
...
#
#
def interface_stats(aCTX, aArgs = None):
...
def vm_mapping(aCTX, aArgs = None):
 ...
 TODO, save all interfaces for non-matched-interfaces, i.e the inventory VMs, to another list and run a database check against them because they should be there... update interface table :-)
...
#
#
def log(aCTX, aArgs = None):
 """Function docstring for log. Insert device log
