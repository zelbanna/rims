...
def rest(aDict):
 ...
 try:
  if aDict.get('href'):
   ...
   with DB() as db:
    db.do("SELECT node_url, service_port, service_url FROM openstack_tokens LEFT JOIN openstack_services ON openstack_tokens.id = openstack_services.id WHERE openstack_tokens.token = '%s' AND service = '%s'"%(aDict['token'],aDict.get('service')))
    data = db.get_row()
...
def call(aDict):
 ...
 """
 with DB() as db:
  db.do("SELECT node_url, service_port, service_url FROM openstack_tokens LEFT JOIN openstack_services ON openstack_tokens.id = openstack_services.id WHERE openstack_tokens.token = '%s' AND service = '%s'"%(aDict['token'], aDict['service']))
  data = db.get_row()
 controller = Device(data['node_url'],aDict['token'])
 try:
...
def vm_console(aDict):
 ...
 with DB() as db:
  db.do("SELECT node_url, service_port, service_url FROM openstack_tokens LEFT JOIN openstack_services ON openstack_tokens.id = openstack_services.id WHERE openstack_tokens.token = '%s' AND service = 'nova'"%(aDict['token']))
  data = db.get_row()
 controller = Device(data['node_url'],aDict['token'])
 try:
...
def vm_resources(aDict):
 ...
 ret = {}
 with DB() as db:
  db.do("SELECT node_url,id FROM openstack_tokens WHERE token = '%s'"%(aDict['token']))
  data = db.get_row()
...
def contrail_fqname(aDict):
 ...
 """
 with DB() as db:
  db.do("SELECT node_url FROM openstack_tokens WHERE token = '%s'"%aDict['token'])
  data = db.get_row()
 controller = Device(data['node_url'],aDict['token'])
 try:
